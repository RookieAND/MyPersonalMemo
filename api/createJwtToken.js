...
export const jwtMiddleWare = (res, req, next) => {
    ...
    try {
        ...
        // 만약 토큰 만료 기간까지 3시간도 남지 않았다면, 이를 재발급 합니다.
        if (Date.now() / 1000 - decoded.iat > 60 * 60 * 3) {
            const { id, registed } = decoded;
            const freshToken = generateToken({ id, registed });

            // res.cookie(name, content, option) 으로 쿠키를 설정함.
