...
const reducer = (state, action) => {
    switch (action.type) {
        case 'CREATE_CATEGORY':
            if (state.filter((memoList) => memoList.category === action.content.category).length > 0) {
                return;
            }
                ...
                memoList.category === action.category
                    ? {
                          ...
                          memo: [
                              ...
                              {
                                  ...action.newMemo,
                                  id: memoList.memo.length === 0 ? 1 : memoList.memo[memoList.memo.length - 1].id + 1,
                              },
                          ],
                      }
                    : memoList
        ...
        case 'MODIFY_MEMO':
            return state.map((memoList) =>
                memoList.category === action.category
                    ? {
                          ...memoList,
                          memo: memoList.memo.map((memo) => (memo.id === action.id ? action.modifiedMemo : memo)),
                      }
                    : memoList
            );
